define(["require", "exports", "Phaser"], function (require, exports, Phaser) {
    ...
    class Play extends Phaser.Scene {
        constructor() {
            ...
            this.score = 0;
            this.gameOver = false;
            this.arrayToBeDestroyed = []; // place to save the components of the rope so later we can remove the rope with the help of the array
            this.ropeNumber = 0; // a number to help to keep rope number at maximum 1
        }
        create() {
                ...
                for (var i = 0; i < pairs.length; i++) {
                    ...
                    // if the player meet star, the star will be consumed and the score the increase by 10; if all stars disappear, new patch of stars and a bomb will drop
                        ...
                        if (this.stars.bodies.length == 0) {
                            ...
                            var bomb = this.matter.add.image(Phaser.Math.Between(32, 768), 20, 'bomb', Phaser.Math.Between(0, 5), { label: 'redball' });
                            bomb.setBounce(0.99);
                            bomb.setFriction(0);
                            bomb.setFrictionAir(0.01);
                            this.matter.body.setInertia(bomb.body, Infinity);
                    ...
                    // if the end point touches the floor above, the rope will be hinged to the floor above
                        ...
                        var constraintBetweenWorldPointAndLastBall = this.matter.add.worldConstraint(endBall, 10, 1, {
    ...
    Play.hanging = false; // if the player is haning on the rope
    Play.onFloor = false; // if the player is steping on the floor
});
